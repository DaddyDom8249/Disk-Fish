...
import { angleDeg, angDiff, dist2, headingDeg, isVisible, mean } from "./geometry.ts";
  ...
  if (plantFrame && setupFrame && sw) {
    ...
    if (p && r) {
      ...
      out.push(
        m({
          id: "stride_length",
          label: "Plant stance separation",
          category: "lower_body",
          value: v,
          unit: "× shoulder width",
          classification: "measured",
          ...confFromVis(visAt(plantFrame, [plantAnkle, rearAnkle])),
          method: "Horizontal ankle separation at plant, normalized by shoulder width; this is a 2D stance-separation proxy, not true stride length.",
          frame: plantFrame.frame,
          timestampMs: plantFrame.timestampMs,
          normalized: true,
          limitations: "Uses camera-horizontal as a proxy for the throwing plane.",
          howMeasured: "Horizontal distance between plant and rear ankles at plant, normalized by shoulder width.",
        }),
      );
    } else {
